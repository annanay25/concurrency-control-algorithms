...
vector< int > read_fun(int tid)
{
  // cout << "Hello from read_fun() " << tid << endl;
  ...
  for(vector<op_info>::size_type i = 0; i != op_list.size(); i++) {
    ...
    if(op_list[i].tin == tid && op_list[i].rw == 'r'){
      // cout << "In read_fun(): " << op_list[i].rw << op_list[i].tin << op_list[i].var << endl;
      ret.push_back(i);
    }
...
void *CSTest(void *argStruct){
  ...
  vector< int > read_set = read_fun(args.tid + 1);
